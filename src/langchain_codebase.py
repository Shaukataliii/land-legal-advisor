...
def create_or_load_chroma_vectorstore(database_path, docs):
    if os.path.exists(database_path):
        vector_db = create_vectorstore(docs, database_path)
    else:
        vector_db = load_vectorstore(database_path)
    
    return vector_db

def create_vectorstore(docs, database_path: str, embedding_modelname: str = 'models/embedding-001'):
    print("Creating and saving embeddings..")
    embedding_model = GoogleGenerativeAIEmbeddings(model=embedding_modelname)
    return Chroma.from_documents(documents=docs, embedding=embedding_model, persist_directory=database_path)

def load_vectorstore(database_path: str, embedding_modelname: str = 'models/embedding-001'):
    if is_valid_path(database_path):
        print("Loading saved embeddings..")
        embedding_model = GoogleGenerativeAIEmbeddings(model=embedding_modelname)
        return Chroma(persist_directory=database_path, embedding_function=embedding_model)
...
def create_rag_chain_from_vdb_path_using_google_embedding(vectorstore_name: str, system_role: str):
    print("Loading retriever..")
    vectorstore = load_vectorstore(vectorstore_name)
    simple_retriever = convert_vectorstore_to_retriever(vectorstore)
    return create_rag_chain_from_retriever(simple_retriever, system_role)
...
def create_rag_chain_from_retriever(simple_retriever, system_role):
...
def hit_chain_get_response_with_history(rag_chain, query, history):
    response = rag_chain.invoke({"input": query, "chat_history": history})
    save_response(response)
